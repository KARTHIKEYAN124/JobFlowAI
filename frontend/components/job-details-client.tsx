...
type Job = { id: string; title: string; company_name: string; location: string; remote_type: string; employment_type: string; category: string; posted_at: string; description: string; application_url: string; source: string; required_skills: string[]; preferred_skills: string[]; language_requirements: Record<string, string> };
type Match = { job_id: string; overall_score: number; matched_skills: string[]; missing_skills: string[]; explanation: string };
type Insights = { recommendations: string[]; provider: string };
...
export function JobDetailsClient({ jobId }: { jobId: string }) {
  ...
  useEffect(() => {
    void Promise.all([api<Job>(`/jobs/${jobId}`), api<Match[]>("/matches"), api<Insights>("/ai/job-insights", { method: "POST", body: JSON.stringify({ job_id: jobId }) })]).then(([record, matches, jobInsights]) => { setJob(record); setMatch(matches.find(item => item.job_id === jobId) ?? null); setInsights(jobInsights); }).catch(caught => setError(caught instanceof Error ? caught.message : "Could not load this job."));
  }, [jobId]);
