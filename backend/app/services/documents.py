...
def tailored_resume_pdf(profile,resume,job):
    ...
    if not relevant: relevant=[skill for skill in profile.skills if skill.lower() in job.description.lower()]
    styles=getSampleStyleSheet(); output=BytesIO()
    document=SimpleDocTemplate(output,pagesize=A4,rightMargin=18*mm,leftMargin=18*mm,topMargin=16*mm,bottomMargin=16*mm,title=f"{profile.full_name or 'Candidate'} - tailored resume")
    story=[Paragraph(escape(profile.full_name or "Candidate"),styles["Title"])]
    if profile.headline: story.extend([Paragraph(escape(profile.headline),styles["Heading2"]),Spacer(1,4*mm)])
    story.extend([Paragraph("Relevant verified skills",styles["Heading2"]),Paragraph(escape(", ".join(relevant) or "See verified resume content below."),styles["BodyText"]),Spacer(1,5*mm),Paragraph("Verified resume content",styles["Heading2"])])
    for line in (resume.extracted_text or "").splitlines():
        if line.strip(): story.extend([Paragraph(escape(line.strip()),styles["BodyText"]),Spacer(1,1.5*mm)])
